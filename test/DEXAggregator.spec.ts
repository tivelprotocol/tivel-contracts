import { abi as UNIV2ROUTER_ABI } from "@uniswap/v2-periphery/build/UniswapV2Router02.json"
...
import { forkNetwork, deployContract, ThenArgRecursive, bestUniswapV3AmountOut, bestUniswapV3AmountIn, bestUniswapV2AmountIn, bestUniswapV2AmountOut } from "./shared/helpers";
...
async function fixture() {
    ...
    const univ2Router = await getUniV2Router();
    const univ2Integration = await deployContract(deployer, "UniswapV2DEXIntegration", [uniswapv2.factory, uniswapv2.router]);
...
async function getUniV2Router() {
    return ethers.getContractAt(UNIV2ROUTER_ABI, uniswapv2.router);
}
    ...
    context("arbitrum", () => {
        ...
        context("207259510", async () => {
            ...
            beforeEach(async () => {
                ...
                await fix.aggregator.addDEX(fix.univ2Integration.address, "UniswapV2")
                ...
                expect(await fix.aggregator.dexes(0)).equals(fix.univ2Integration.address)
                expect(await fix.aggregator.dexNames(0)).equals("UniswapV2")
                expect(await fix.aggregator.dexIndex(fix.univ2Integration.address)).equals(BigNumber.from("1"))
                expect(await fix.aggregator.dexes(1)).equals(fix.univ3Integration.address)
                expect(await fix.aggregator.dexNames(1)).equals("UniswapV3")
                expect(await fix.aggregator.dexIndex(fix.univ3Integration.address)).equals(BigNumber.from("2"))
            });
            ...
            it("removeDEX then addDEX", async () => {
                ...
                expect(await fix.aggregator.dexLength()).equals(BigNumber.from("3"))
                expect(await fix.aggregator.dexes(0)).equals(fix.univ2Integration.address)
                expect(await fix.aggregator.dexNames(0)).equals("UniswapV2")
                expect(await fix.aggregator.dexIndex(fix.univ2Integration.address)).equals(BigNumber.from("1"))
                expect(await fix.aggregator.dexes(1)).equals(fix.univ3Integration.address)
                expect(await fix.aggregator.dexNames(1)).equals("UniswapV3")
                expect(await fix.aggregator.dexIndex(fix.univ3Integration.address)).equals(BigNumber.from("2"))
                expect(await fix.aggregator.dexes(2)).equals("0x0000000000000000000000000000000000000000")
                expect(await fix.aggregator.dexNames(2)).equals("Test DEX")
                expect(await fix.aggregator.dexIndex("0x0000000000000000000000000000000000000000")).equals(BigNumber.from("3"))
                ...
                expect(await fix.aggregator.dexNames(1)).equals("UniswapV3")
                expect(await fix.aggregator.dexIndex(fix.univ3Integration.address)).equals(BigNumber.from("2"))
                await expect(fix.aggregator.dexes(2)).to.be.reverted
                await expect(fix.aggregator.dexNames(2)).to.be.reverted
            ...
            it("getAmountOut: 0.01 WETH => USDT", async () => {
                ...
                const i0Result = await bestUniswapV2AmountOut(fix.univ2Router, tokenIn, tokenOut, amountIn)
                const i1Result = await bestUniswapV3AmountOut(fix.univ3Quoter, tokenIn, tokenOut, amountIn)
                const reference = [i0Result, i1Result].sort((a, b) => {
                    return a.gt(b) ? -1 : 1
                })[0]
            ...
            it("getAmountIn: WETH => 1 USDT", async () => {
                ...
                const i0Result = await bestUniswapV2AmountIn(fix.univ2Router, tokenIn, tokenOut, amountOut)
                const i1Result = await bestUniswapV3AmountIn(fix.univ3Quoter, tokenIn, tokenOut, amountOut)
                const reference = [i0Result, i1Result].sort((a, b) => {
                    return a.lt(b) ? -1 : 1
                })[0]
            ...
            it("swap: 0.01 WETH => USDT then 1 USDT => WETH", async () => {
                ...
                const i0Result1 = await bestUniswapV2AmountOut(fix.univ2Router, tokens.weth, tokens.usdt, amountIn1)
                const i1Result1 = await bestUniswapV3AmountOut(fix.univ3Quoter, tokens.weth, tokens.usdt, amountIn1)
                const reference1 = [i0Result1, i1Result1].sort((a, b) => {
                    return a.gt(b) ? -1 : 1
                })[0]
                ...
                const i0Result2 = await bestUniswapV2AmountOut(fix.univ2Router, tokens.usdt, tokens.weth, amountIn2)
                const i1Result2 = await bestUniswapV3AmountOut(fix.univ3Quoter, tokens.usdt, tokens.weth, amountIn2)
                const reference2 = [i0Result2, i1Result2].sort((a, b) => {
                    return a.gt(b) ? -1 : 1
                })[0]
